 ...
 * Stores logs in browser.storage.local with 7-day retention policy.
 */
...
export async function flushLogs(immediate: boolean = false): Promise<void> {
    ...
    try {
        ...
        // 既存ログを取得
        const storage = await browser.storage.local.get(LOG_STORAGE_KEY);
 ...
 */
if (typeof chrome !== 'undefined' && browser.runtime && browser.runtime.onSuspend) {
...
/**
 * CRITICAL — 構造化ERRORログ + browser.notifications アラート
...
export async function logCritical<T extends object = Record<string, unknown>>(
    ...
    try {
        if (typeof chrome !== 'undefined' && browser.notifications && typeof browser.notifications.create === 'function') {
            const title = browser.i18n.getMessage('criticalAlertTitle') || 'Yasumaro — Critical Error';
            const notificationMessage = browser.i18n.getMessage('criticalAlertBody', [message]) || message;
            const iconUrl = (typeof browser.runtime !== 'undefined' && typeof browser.runtime.getURL === 'function')
                ? browser.runtime.getURL('/icons/icon48.png')
                : 'icons/icon48.png';
